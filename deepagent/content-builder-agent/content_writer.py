...
def _resolve_example_dir() -> Path:
    candidates = [
        Path("."),
        Path("deepagent/content-builder-agent"),
        Path(".."),
        Path("../deepagent/content-builder-agent"),
    ]
    for candidate in candidates:
        if (candidate / "subagents.yaml").exists():
            return candidate

    searched = ", ".join(str(p) for p in candidates)
    raise FileNotFoundError(f"Could not find subagents.yaml in relative paths: {searched}")
...
def _patch_asyncio_create_task_for_legacy_loops() -> None:
    """Compat for environments where loop.create_task lacks `context` kwarg."""
    create_task = asyncio.create_task
    if getattr(create_task, "_context_compat_patched", False):
        return

    def _create_task_compat(coro, *, name=None, context=None):
        try:
            return create_task(coro, name=name, context=context)
        except TypeError as e:
            if "unexpected keyword argument 'context'" not in str(e):
                raise
            if name is not None:
                return create_task(coro, name=name)
            return create_task(coro)

    _create_task_compat._context_compat_patched = True
...
async def main():
    ...
    _patch_asyncio_create_task_for_legacy_loops()
